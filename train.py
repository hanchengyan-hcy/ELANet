...
class Train(object):
    ...
    def train(self):
        ...
        if os.path.exists(args.model_path + args.model):
            ...
            self.train_loss_3 = state['train_loss_3']
            self.lr = state['lr']
        ...
        for ep in range(self.num_epochs):
            ...
            for batch, (over, under) in enumerate(self.train_loader):
                ...
                loss_2 = weight_list[:, 0] * self.loss_mse(fused_img, over) \
                         + weight_list[:, 1] * self.loss_mse(fused_img, under)  # '\'为续行符
                loss_2 = torch.mean(loss_2)

                # TV loss
                loss_3 = weight_list[:, 0] * self.loss_TV(fused_img, over) \
                         + weight_list[:, 1] * self.loss_TV(fused_img, under)
                loss_3 = torch.mean(loss_3)

                # total loss
                loss = (loss_1 + 20 * loss_2 + 20 * loss_3) * 1000
                ep_loss_1.append(loss_1.item())
                ep_loss_2.append(loss_2.item())
                ep_loss_3.append(loss_3.item())
                loss.backward()
                self.optimizer.step()

                if batch % 50 == 0 and batch != 0:
                    print('Epoch:{}\tcur/all:{}/{}\tLoss_1:{:.4f}\tLoss_2:{:.4f}\tLoss_3:{:.4f}\tLoss:{:.4f}\t'
            ...
            self.train_loss_3.append(np.mean(ep_loss_3))

            state = {
                'model': self.model.state_dict(),
                'train_loss_1': self.train_loss_1,
                'train_loss_2': self.train_loss_2,
                'train_loss_3': self.train_loss_3,
            ...
            fig3 = plt.figure()
            plot_loss_list_3 = self.train_loss_3
            plt.plot(plot_loss_list_3)
            plt.savefig('train_loss_curve_3.png')

            writer.add_scalar('ssim_loss', np.mean(ep_loss_1), ep)
            writer.add_scalar('mse_loss', np.mean(ep_loss_2), ep)
            writer.add_scalar('TV_loss', np.mean(ep_loss_3), ep)
